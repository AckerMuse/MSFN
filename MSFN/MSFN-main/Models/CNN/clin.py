...
clin = pd.read_csv("../../Data/Clin_33.csv", index_col=False, header=None).astype('float').values
...
for idx_train, idx_val,in kfold.split(clin, labels):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_data, batch_labels in dataloader:
            ...
            if len(np.unique(batch_labels.cpu().detach().numpy())) == 1:
...
np.savetxt("../../Outputs/CLin_feature.csv",stacked_feature,delimiter=',')
